...
async function analyzeEmail() {
  try {
    ...
    try {
      ...
      // Improved fallback - tries multiple selectors for Gmail & Outlook
      const [result] = await chrome.scripting.executeScript({
        ...
        func: () => {
          ...
          // Try Outlook
          if (!sender) {
            sender = document.querySelector(".ms-Persona-primaryText")?.innerText 
                  || document.querySelector("[data-testid='messageHeader']")?.innerText?.split("\n")[0]
                  || "";
          }
    ...
    // Save to history WITH subject & sender (so dashboard shows them correctly)
    chrome.storage.local.get("scan_history", (res) => {
      const history = res.scan_history || [];
      const record = {
        ...data,
